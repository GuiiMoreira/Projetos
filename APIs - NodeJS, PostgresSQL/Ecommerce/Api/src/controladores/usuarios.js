...
const cadastrarUsuario = async (req, res) => {
    ...
    try {
        const quantidadeUsuarios = await knex('usuarios').where('email', email).debug();
...
const atualizarPerfil = async (req, res) => {
    ...
    try {
        ...
        const usuarioAtualizado = await knex('usuarios').where('id', req.usuario.id).update({nome, nome_loja, email, senha }).debug()
